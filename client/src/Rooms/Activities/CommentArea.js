...
import { Box, Typography, Avatar, Button, Divider, Input } from '@mui/material';
...
function CommentArea({ socket, commentId, activityID }) {
  ...
  useEffect(() => {
    axios
      .post('http://localhost:5000/activity/get/comment', { activityID })
      .then((res) => {
    ...
    >
      ...
      >
        {JSON.parse(localStorage.userData).data.user.userType === 'Professor'
          ...
          : 'Send your concerns here about this activity'}
      </Typography>
                    ...
                    <Box className="User-date" sx={{ margin: '3px 0px' }}>
                      <Box>
                        ...
                        >
                          ...
                          >
                            {moment(items[0].commentDate).format('LL')}
                          </Typography>
                        </Box>
                      </Box>
                      <Box
