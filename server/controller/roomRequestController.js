...
const newRequests = async (req, res) => {
  try {
    ...
    if (!room.members.includes(req.body.userID)) {
      const requests = new RoomRequestModel({
      ...
      return res.json({
        message: 'Your request is sent. Please come again later',
      });
    }
    if (room.members.includes(req.body.userID)) {
      return res.json({
...
const acceptRequest = async (req, res) => {
  try {
    ...
    await RoomRequestModel.findByIdAndDelete(req.body.reqID);
    const requests = await RoomRequestModel.find({
      hostID: req.body.userID,
    }).sort({ createdAt: -1 });
    const gs = await GradingSystemModel.find({
      rooms: { $elemMatch: { $eq: req.body.roomID } },
    });
    const initialGrade = [];
    gs[0].Category.map((item) => {
      initialGrade.push({ [Object.entries(item)[0][0]]: 0 });
    });
    await ClassRecordModel.updateOne(
      { room: { $elemMatch: { $eq: req.body.roomID } } },
      {
        gradingSystem: initialGrade,
      }
    );
    console.log(gs[0]._id);
