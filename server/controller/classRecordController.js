      ...
      req.body.scores.forEach(async (scores) => {
        ...
        recordItem.gradingSystem.forEach(item => {
          if (Object.entries(item)[0][0] === activityCategory.gsCategory) {
            grade[recordItem.student.stdID].push({ [Object.entries(item)[0][0]]: (parseInt(Object.entries(item)[0][1]) + parseInt(scores.score)) / oldActivities.length })
          } else if (Object.entries(item)[0][0] !== activityCategory.gsCategory) {
...
const getOverall = async (req, res) => {
  try {
    ...
    gsData[0].Category.forEach(item => {
      percent = { ...percent, [Object.entries(item)[0][0]]: Object.entries(item)[0][1] }
      //console.log(item)
    })
    let overall = {}
    recordData.forEach(students => {
      ...
      students.gradingSystem.forEach((item) => {
        overall[students.student.stdID] += (parseFloat(percent[Object.entries(item)[0][0]]) * (item[Object.entries(item)[0][0]] / 100))
